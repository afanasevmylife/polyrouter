...
function scanInput(input: unknown): {
	...
	if (typeof input === "string") return { text: input, turns: 1, hasImage: false };
 ...
 */
export function aggregateHealth(statusRows: StatusRow[]): Map<string, ModelHealth> {
	const acc = new Map<
		string,
...
const isUsable = (
	...
	if (tierRequirement === "deep" && !model.capabilities?.includes("reasoning"))
		return "no reasoning";
	return null;
};
...
export function pickModel(
	...
	for (const wanted of order) {
		...
		const skipTrace = [...skipped.entries()]
			...
			.join(", ");
		const escalated =
			wanted !== features.tier ? `escalated ${features.tier}->${wanted ?? "any"}; ` : "";
		const why =
	...
	throw new Error("No eligible text model in the catalog can serve this request");
}
